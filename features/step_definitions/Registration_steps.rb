...
Then('I sign in with a valid email and OTP') do
	...
	if $web_driver.find_elements(ElementWarehouse::SIGN_IN).first
		$web_driver.find_elements(ElementWarehouse::SIGN_IN).first.click
		$web_driver.switch_to.window( $web_driver.window_handles.last )
	end
	...
	CODE = $web_driver.find_element(ElementWarehouse::VERIFICATION_CODE).get.text
	sleep 2
