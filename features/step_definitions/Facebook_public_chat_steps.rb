...
Then ('I create a facebook post')do
	...
	sleep 2
	# $web_driver.find_element(ElementWarehouse::FACEBOOK_NEXT_BUTTON).click
	sleep 4
	...
	$web_driver.find_element(ElementWarehouse::SEARCH_FIELD).type('QA 9020 2')
	sleep 10
	$web_driver.find_element(ElementWarehouse::PAGE_OPTION_2).click
	sleep 10
	if $web_driver.find_elements(ElementWarehouse::FACEBOOK_PAGE).first
		$web_driver.find_elements(ElementWarehouse::FACEBOOK_PAGE).first.click
	elsif $web_driver.find_elements(ElementWarehouse::FACEBOOK_PAGE_2).first
